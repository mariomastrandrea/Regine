...
import it.polito.tdp.regine.model.MyRegineModel;
...
public class FXMLController 
{
	...
	void handleSolve(ActionEvent event) 
	{
		...
		regine = new MyRegineModel();
		Set<Integer[]> soluzioni = regine.computeCombinations(N);
		Iterator<Integer[]> iterator = soluzioni.iterator();
		Integer[] primaSoluzione = iterator.next();

		// "Dipingi" la soluzione
		if (primaSoluzione != null) 
		{
			for (int col = 0; col < N; col++) 
			{
				int row = primaSoluzione[col];
